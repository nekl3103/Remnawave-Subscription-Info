...
- Запоминает SSH адрес и пароль роутера на время текущего запуска.
- Обновляет `/etc/sing-box/config.json`.
- Создает timestamp backup `/etc/sing-box/config.json.bak.YYYYMMDD-HHMMSS` и восстанавливает последний backup.
- Перезапускает `sing-box`.
- Показывает статус `sing-box`.
- Показывает логи `sing-box` в реальном времени.
- Поддерживает поиск, фильтр серверов и обновление списка из подписки.
- Экспортирует готовый `outbounds` в JSON файл.
- Позволяет сбросить сохраненные SSH данные текущего запуска.
...
Программа обновляет только outbound с `tag: "proxy"` и не удаляет остальные outbounds. Перед заменой основного конфига новый файл проверяется командой:

```text
sing-box check -c <temporary-config>
```

Если проверка не прошла, основной конфиг не заменяется. После успешного сохранения программа перезапустит `sing-box`. Восстановление использует самый свежий backup и поддерживает старый файл `/etc/sing-box/config.json.bak`, если timestamp backup еще не создавался.
